...
chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
    ...
    // 创建全局浮动窗口
    if (request.action === 'createGlobalFloatingWindow') {
        try {
            const floatingUrl = chrome.runtime.getURL('floating-manager.html');
            ...
            chrome.windows.getAll({ populate: true }, (windows) => {
                ...
                for (const window of windows) {
                    for (const tab of window.tabs) {
                        if (tab.url && tab.url.includes('floating-manager.html')) {
                            floatingWindow = window;
                            break;
                        }
                    }
                    if (floatingWindow) break;
                }
                
                if (floatingWindow) {
                    // 如果已有浮动窗口，聚焦并置顶
                    chrome.windows.update(floatingWindow.id, { 
                        focused: true,
                        drawAttention: true 
                    }, () => {
                        sendResponse({ success: true, action: 'focused', windowId: floatingWindow.id });
                    });
                } else {
                    // 计算窗口位置（右上角）
                    chrome.system.display.getInfo((displays) => {
                        ...
                        const leftPosition = screenWidth - windowWidth - 60;
                        ...
                        chrome.windows.create({
                            ...
                            if (chrome.runtime.lastError) {
                                console.error('Error creating global floating window:', chrome.runtime.lastError);
            ...
            console.error('Error creating global floating window:', error);
